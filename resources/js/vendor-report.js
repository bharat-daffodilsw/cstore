cstore.controller('vendorReportCtrl', function ($scope, $appService, $location) {
    ...
    $scope.filterdata.selectedState="";
    $scope.venderSearch = [
...
cstore.directive('vendorReport', ['$appService', function ($appService, $scope) {
    return {
        ...
        template: '<div class="add_delete pull-left"><div class="search_by pull-left">Search By<search-by></search-by></div>' +
            ...
            '<div class="search_sign_2 pull-left"><a ng-click="search()"><img style="cursor: pointer" src="images/Search.png"></a></div><input type="submit" style="display:none;"></form></div><div class="delete_btn pull-right"><button type="button" ng-click="getExportVendors()"><a>Download in Excel</a></button></div>' +
            '<div class="delete_btn pull-right"><button type="button" ng-click="printDiv(\'printVendors\')">Print</button></div>' +
            '<div class="delete_btn pull-right"><button type="button" ng-click="generatepdf()">Download in PDF</button></div></div>'+
            ...
            return {
                ...
                post:function($scope){
                    $scope.generatepdf=function(){
                        ...
                        $scope.vendorpdfurl = BAAS_SERVER + "/export/pdf?query=" + JSON.stringify(pdfquery) + "&ask=" + ASK + "&osk=" + OSK + "&templateId="+tempalateId+"&state="+JSON.stringify({"timezone": timeZone});
