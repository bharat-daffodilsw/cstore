...
cstore.directive('orderReport', ['$appService', function ($appService, $scope,$window) {
    return {
        ...
        template: '<div class="add_delete pull-left">' +
            ...
            '<tr ng-repeat="pop in order.product"><td class="ordered_pop pop_name">{{pop.name}}</td><td class="ordered_pop">{{pop.cost.amount | currency}}</td><td class="ordered_pop">{{pop.quantity}}</td></tr></table></td><td>' +
